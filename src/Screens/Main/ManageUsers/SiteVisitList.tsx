...
export default function SiteVisitList() {
  ...
  const { values, initialValues, setFieldValue, handleSubmit, resetForm } =
    useFormik({
      ...
      onSubmit(values) {
        setFilters(values);
        GetSiteVisitList(1, size, values);
      },
    });
  const [filters, setFilters] = useState(initialValues);

  const GetSiteVisitList = (
    page = 1,
    size = 10,
    ...
    ListSiteVisitRequestService(page, size, finalObj)
      .then((response) => {
  ...
  const Options: TableOptionsType[] = [
...

    {
      lable: "Visitor Type",
      render: (text) =>
        VisitorTypeOptions?.find((ele) => ele?.value === text)?.label || "-",
      key: "visitor_type",
      className: classes.NameBox,
    },
    {
    ...
    },
    {
      ...
      render: (text, data) => (
        <TableActionBlock
          permissionData={{ view: 1 }}
          onClickViewIcon={() => {
            setViewModal({ id: data?.visit_record_id, show: true });
          }}
        />
      ),
    },
  ];
  const [viewModal, setViewModal] = useState({ id: 0, show: false });

  useEffect(() => {
    if (token) {
      GetSiteVisitList(page, size, values);
    }
  ...
  return (
    <>
      ...
      {viewModal.show && (
        <GlobalModal
          OnClose={() => {
            setViewModal({ show: false, id: 0 });
          }}
          title="View"
          Visible={viewModal.show}
          size={1000}
        >
          <ViewSiteVisitModal UpdateData={viewModal.id} />
        </GlobalModal>
      )}
      <ScreenHeader
        name={`Site Visit Record`}
        OnClickFilter={() => {
      ...
      <div className={classes.bgContainer}>
        ...
        <div className={classes.tablecontainer}>
          ...
          <CommonPaginaion
            DataList={DataList}
            handleListapi={GetSiteVisitList}
            filters={filters}
          />
        </div>
      </div>
